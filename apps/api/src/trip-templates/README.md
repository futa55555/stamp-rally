...
- genre名は共通表記（例：グルメ・景色・温泉・アウトドア・まち歩き・おみやげ）を使ってください。同じgenre内で同じ `title` のstampは1件にまとまります。genreが異なる同名stampはそれぞれ選択できます。Tripの作成時には同じtitleを1つのstampにまとめ、選択されたgenreすべてに所属させます。一方のgenreだけで選択すれば、そのgenreだけに所属します。既存stampや手動作成の同名stampは統合しません。
...
例えば「パークならではのフードを食べる」はグルメ・遊園地の両方に記載します。「夜景を楽しむ」は景色・思い出の両方で使います。同じ体験を複数genreで扱うときはtitleを統一し、別々に達成したい体験（フードとドリンクなど）は別のtitleを維持してください。
