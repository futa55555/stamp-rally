...
4. 写真詳細の横スワイプ・ズーム、動画再生、お気に入り、原本の共有・保存、既読、削除後の達成状態を確認する。
5. HEIC・HEVCを含む複数投稿、通信中断とアプリ復帰後の再開、個別失敗・キャンセル、他の参加者への公開、動画通知からの遷移を確認する。実機の権限とOSシートはunit／exportでは検証できません。
