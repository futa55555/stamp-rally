...
const post: Post = {
  ...createDemoData().posts[0],
  smallUrl: 'https://storage.example/small.webp?X-Amz-Signature=old',
};
const fresh: Post = {
  ...post,
  smallUrl: 'https://storage.example/small.webp?X-Amz-Signature=new',
};
...
it.each(['list only', 'active detail'])(
  ...
  async (state) => {
    const queryKey = resourceKey('viewer', `/posts/${post.id}`);
    if (state === 'active detail') {
      const observer = new QueryObserver(cache, {
        queryKey,
        initialData: post,
        queryFn: () => native.request({ url: `/posts/${post.id}` }),
      });
    ...
    expect(url).toBe(fresh.smallUrl);
