...
export function InviteScreen() {
  ...
  if (!linkId && !isInvitationToken(token))
    return (
      <Screen contentContainerClassName="px-4 pt-6">
        ...
        <Button label="閉じる" onPress={cancel} />
      </Screen>
    );
  if (!user || user.status === 'ONBOARDING')
    return (
      <>
        ...
        <Button label="招待を閉じる" variant="secondary" onPress={cancel} />
      </>
    );
  ...
  return (
    <Screen {...refresh} contentContainerClassName="px-4 pt-6">
      ...
      {preview.linkStatus === 'EXPIRED' || preview.linkStatus === 'REVOKED' ? (
        <AppText tone="textSecondary">
          この招待リンクは期限切れ、または無効化されています。新しく参加を申請するには、新しい招待リンクを受け取ってください。
        </AppText>
      ) : null}
      {preview.isMember ? (
        <Button
      ...
      {preview.invitation && !preview.canRequest ? (
        <InvitationControls
