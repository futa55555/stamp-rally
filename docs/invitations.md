...
Androidは `pnpm mobile:start` / `pnpm mobile:android` / `pnpm mobile:ios` のローカル起動中にAPI・Webのポートを開発PCへ自動転送するため、同じ `localhost` 設定を使えます。Metroのポート転送はExpoが行います。起動手順は [Androidのローカル開発](android-development.md) を参照してください。
